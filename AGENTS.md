...
- Для device-level smoke добавлен selector smoke-driver `scripts/android-two-device-smoke.mjs` и runbook в `docs/project/runbooks.md`; driver использует UIAutomator `resource-id` из test tags, а не жёстко заданные tap-координаты, и прошёл на `emulator-5554` + `emulator-5556` после очистки app state и backend DB.
- Текущие ограничения: полный сценарный two-device smoke A-D через UI пока не автоматизирован и не считается пройденным; ADB daemon на Windows может падать и требует `wadb kill-server/start-server`; фото picker и deterministic conflict setup требуют отдельного сценарного драйвера или ручного runbook шага. Backend PostgreSQL Testcontainers tests skip локально без Docker daemon, но выполняются при доступном Docker.
- Следующий шаг перед первой beta: расширить selector driver до full A-D или пройти A-D вручную строго по tags из runbook на двух реальных телефонах/эмуляторах, включая photo sync и conflict resolver, затем фиксировать beta readiness.
