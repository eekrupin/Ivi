...
- Фото питомца через системный Android Photo Picker с копированием во внутреннее хранилище и синхронизацией через отдельный backend binary API.
...
- миграция `6 -> 7` добавляет `pets.photoRevision` для отслеживания серверной ревизии фото;
- destructive migration не использовать как способ чинить обновления поверх старой debug-базы.

Фото питомца в Android хранится локально как `photoUri`, а серверная актуальность определяется через `photoRevision`. После bootstrap/changes отдельный coordinator скачивает недостающую ревизию фото или очищает локальную серверную фотографию, если на сервере фото удалено. При сохранении профиля питомца Android пытается загрузить/удалить фото на сервере, но локальное сохранение остается local-first и не откатывается при сетевой ошибке.
