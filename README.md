...
На текущем этапе проект развивается как локальный MVP без backend и синхронизации между устройствами.
...
- JDK `17`
- Android SDK `35`
- Для сборки через wrapper используется `./gradlew`
...
В проекте уже подготовлена настройка release signing через файл `keystore.properties` в корне репозитория.
...
Проект пока состоит из одного модуля `:app`, внутри которого соблюдается разделение на слои:
